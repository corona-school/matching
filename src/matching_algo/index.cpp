...
Napi::Value Match(const Napi::CallbackInfo& info) {
    ...
    //for the first 4 arguments, check whether this file exists or not
    for (int i = 0; i < MATCH_ALGO_ARG_COUNT - 1; i++) {
        //check if file exist
    ...
    //for the last argument, make sure that the parent folder exists and the path is really a filename path (not a path to a directory)
    std::string outputFileName = info[MATCH_ALGO_ARG_COUNT - 1].As<Napi::String>();
    ...
    performAlgorithmWithFilenames(arg0.c_str(), arg1.c_str(), arg2.c_str(), arg3.c_str(), arg4.c_str());
